...
tertimumNonDbool : (b : Bool) → atom ( (not b) ∨b b)
tertimumNonDbool false = tt
tertimumNonDbool true = tt

doubleNotAtom₁ : (b : Bool) → atom b → atom (not (not b))
doubleNotAtom₁ true p = tt

doubleNotAtom₁' : (b : Bool) → b ≡  not (not b)
doubleNotAtom₁' false = refl
doubleNotAtom₁' true = refl
...
orEq : (b b' c c' : Bool) → b ≡ b' → c ≡ c' → b ∨b c ≡ b' ∨b c'
orEq b b' c c' refl refl = refl

andEq : (b b' c c' : Bool) → b ≡ b' → c ≡ c' → b ∧b c ≡ b' ∧b c'
andEq b b' c c' refl refl = refl
...
∧b-intro : (b1 b2 : Bool) → atom b1 → atom b2 → atom (b1 ∧b b2)
∧b-intro true true tt tt = tt
