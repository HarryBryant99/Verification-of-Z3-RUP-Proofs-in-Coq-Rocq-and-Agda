...
| Nil -> Nil
| Cons (x, xs) ->
  (match clause_eqb x c with
   | True -> remove_clause c xs
   | False -> Cons (x, (remove_clause c xs)))
...
let rec checkProof p f =
  ...
  | Cons (step, ps) ->
    (match step with
     | Tseitin c -> checkProof ps (app f (Cons (c, Nil)))
     | Rup c ->
       (match rUP_Checker f c with
        | True -> checkProof ps (app f (Cons (c, Nil)))
        | False -> Pair (False, (Some step)))
     | Assumption a -> checkProof ps (app f (Cons (a, Nil)))
