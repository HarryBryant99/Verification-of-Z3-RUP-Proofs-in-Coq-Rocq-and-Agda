...
let rec compare_z3_formula f1 f2 =
  ...
  | Z3ineq x, Z3ineq y -> compare x y
  | Z3eq x, Z3eq y -> compare x y
...
let rec compare_pos_z3_formula p1 p2 =
  match p1, p2 with
  | Pos_z3var v1, Pos_z3var v2 ->
      compare v1 v2

  | Pos_z3ineq i1, Pos_z3ineq i2 ->
      compare i1 i2

  | Pos_z3eq i1, Pos_z3eq i2 ->
      compare i1 i2

  | Pos_and l1, Pos_and l2 ->
      compare_list_z3_formula l1 l2
...
(* Compare two lists of Z3 formulas ignoring order *)
...
(* Strip all outer double negations *)
...
let tseitin_sorter (f_list : listZ3_Formula) (g_list : listZ3_Formula) : bool * tseitinStep option =
  ...
  let find_index_equal_z3 (a : z3_Formula) (lst : listZ3_Formula) : int =
  let rec aux i = function
    | Lnil -> -1
    | Lcons (x, xs) -> if x = a then i else aux (i + 1) xs
  in
  aux 0 lst
...
let coeff_of_var (v : z3_Variable_Int) (vars : linIntExpr) : int =
  match List.assoc_opt v vars with
  | Some c -> c
  | None -> 0

let farkas_step_of_aux (aux : farkasAux) : farkasStep =
  let constraints = aux.constrs in

  (* Collect all variables appearing anywhere *)
  let all_vars =
    constraints
    |> List.concat_map (fun c -> List.map fst c.vars)
    |> List.sort_uniq Int.compare
  in

  (* Build lhs: for each variable, collect its coefficients across constraints *)
  let lhs : listIntExpr =
    List.map
      (fun v ->
         let coeffs =
           List.map
             (fun c -> coeff_of_var v c.vars)
             constraints
         in
         (v, coeffs))
      all_vars
  ...
  (* RHS list *)
  let rhs =
    List.map (fun c -> c.int) constraints
  in
...
let step_processor (s : item) : zProofItem =
  ...
  | FarkasP f ->
      let step = farkas_step_of_aux f in
      (*
      Printf.printf "Farkas step (debug): %s\n"
        (string_of_list_z3_formula (farkas_step_to_clause step));
      *)
      Farkas step
  ...
  | Tseitin (f_list, g_list) ->
      ...
      | (true, true, Some step) ->
          TseitinStep step
      | (false, true, Some step) ->
          TseitinStepRed (step, g_list)
      | _ ->
          failwith "Invalid Tseitin step: could not process"
