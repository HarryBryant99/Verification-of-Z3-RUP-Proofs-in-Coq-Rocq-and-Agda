...
let string_of_list_list_z3_formula (lst : listZ3_Formula list) : string =
  "[" ^ String.concat "; " (List.map string_of_list_z3_formula lst) ^ "]"
...
let string_of_clause_list clauses =
  "[" ^ String.concat "; " (List.map string_of_clause clauses) ^ "]"

(** val rUP_Checker : assumption -> clause -> bool **)

let rUP_Checker a c =
  Printf.printf "Checking with assumption list: %s\n" (string_of_clause_list a);
  Printf.printf "Checking with clause: %s\n" (string_of_clause c);
...
| TseitinStepRed (a, c) ->
  ...
  Printf.printf "Checking with assumption list: %s\n" (string_of_list_list_z3_formula cl);
  Printf.printf "Checking with F list: %s\n" (string_of_list_z3_formula (tseitinProofItem2ConclusionOpt a));
  Printf.printf "Checking with G list: %s\n" (string_of_list_z3_formula c);
  Printf.printf "Checking with setminus: %s\n" (string_of_list_z3_formula (setminus (tseitinProofItem2ConclusionOpt a) c));
  Printf.printf "Checking with neg singletons: %s\n" (string_of_list_list_z3_formula (negSingletons (setminus (tseitinProofItem2ConclusionOpt a) c)));
...
let rec zProofCheckWithFailure = function
  | [] -> Passed
  | x :: xs ->
